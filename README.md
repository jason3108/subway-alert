...
| 版本 | 说明 |
|-----|-----|
| 1.0.37 | 调试信息按钮始终显示，不再需要关闭监控才能访问 |
| 1.0.36 | 轨迹页面集成OSM地图，在地图上显示轨迹路线和标记点 |
| 1.0.35 | 改进闹钟式提醒：添加音频焦点请求，使用异步准备确保铃声可靠播放 |
| 1.0.34 | 后台运行时持续刷新位置监控，使用前台服务在后台保持运行 |
| 1.0.33 | 修复位置轨迹记录问题：首次定位即可保存，优化轨迹记录时机 |
